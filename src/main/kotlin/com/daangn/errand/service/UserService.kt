...
class UserService(
    ...
    fun createOrUpdateUserByDaangnId(userProfile: GetUserProfileRes.Data): Pair<Boolean, User> {
        ...
        val mannerTemp: Float = daangnOpenAPIService.getUserProfile(daangnId).data.user.mannerTemperature ?: 36.5f
    ...
    fun getUserProfileWithDaangnInfo(userId: Long): UserProfileVo {
        ...
        return daangnOpenAPIService.setUserDaangnProfile(userProfileVo)
    }

    @Transactional
    fun getMyProfileWithDaangnInfo(userId: Long, regionId: String): UserProfileVo {
        val user = userRepository.findById(userId).orElseThrow { ErrandException(ErrandError.ENTITY_NOT_FOUND) }
        val userProfileVo = daangnOpenAPIService.setUserDaangnProfile(userConverter.toUserProfileVo(user))
        userProfileVo.regionName = daangnOpenAPIService.getRegionInfoByRegionId(regionId).region.name
